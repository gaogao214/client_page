		...
		[this,fname, recive_len,no_path_added_name](std::error_code ec, std::size_t )
		{
			if (!ec)
			{
				...
				if (ch == '*')
				{
					...
					std::cout << "recive len 1 >: " << filelen << endl;
					if (filelen > 4096 )
					{
				...
				if (filelen > 4096 || ch != '*')//判断大文件，如果收到的文件长度大于4096 就继续接收  或者接收到的sizeof(size_t)这个位置的值不是 * 就继续接收文件
				{
					...
					if (count_ < num)   //判断大文件接没接收完毕     如果接收完毕就继续发送名字
					{
						do_recive_file_text(fname,4096,no_path_added_name);
						//Sleep(1000);//睡眠1毫秒
						printf("\r");
						printf("[ %.2lf%%   ", (count_ * 4096.0 / (filelen - 1)) * 100);  //打印下载了多少的百分比						
					...
					printf("\r");
					printf("[  %.zd%%  ", (filelen / file_size )*100);  //打印下载了多少的百分比						
					printf("] 下载成功  %s ", fname.c_str());
					cout << "\n";
					save_location(fname, no_path_added_name);
					do_send_filename();
			...
			wcfi=dj.parse_wget_c_file_json(dj.wget_c_file1);
...
void down_block::server(const std::string& server_port)//客户端转换成服务端
{
 ...
 void down_block::gsh(std::string& strtxt)//按照格式写入 json 文件
        {

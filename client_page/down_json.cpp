...
void  down_json::parse_down_jsonfile(string& name)//打开配置文件，并找到配置文件中的路径,查看路径下的文件或文件名   解析json文件
{
...
void  down_json::parse_server_list_json(string& text_json)//打开list_json   json文件  解析json文件
{
	files_inserver.deserializeFromJSON(text_json.c_str());
}

void  down_json::parse_block_json(string& text_json)//打开list_json   json文件  解析json文件
{
	blks_.deserializeFromJSON(text_json.c_str());
}

filestruct::wget_c_file_info  down_json::parse_wget_c_file_json(string& name)//打开断点续传文件  解析json文件
{
	string readbuffer = open_json_file(name);
